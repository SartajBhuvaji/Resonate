...
def show_transcript_block():
    ...
    transcript_text.text_area("Transcript", value=text_df, height=len(df) + 200)
    st.markdown(
        """
    <style>
    textarea {
    ...
    if st.button("Update Transcript"):
        ...
        transcript_text.text_area("Transcript", value = updated_line,  height=len(df) + 200)
...
def view1():
    ...
    #st.session_state.new_file_uploaded = True # For testing purposes
    if not st.session_state.new_file_uploaded: 
        upload_files() # sets new_file_uploaded to True if file is uploaded

    new_file_uploaded = st.session_state.new_file_uploaded

    if new_file_uploaded:
        show_transcript_block()
