...
class DeepEmbeddingClustering(object):
    def __init__(self, args, n_clusters, alpha=1.0, cluster_centroid=None):
        ...
        self.alpha = alpha
        self.pretrained_weights_path = os.path.join(args.save_weight_path, "ae_weights_{}.h5")
    ...
    def initialize(self, args, x_data,text, layerwise_pretrain_iters=50000, finetune_iters=100000):
        if not os.path.isfile(self.pretrained_weights_path):
            ...
            [train_log, lr_schedule, early_stopping] = utils.get_callbacks_ae(args)
            ...
            self.autoencoder.fit(x_data, x_data,
                                 ...
                                 callbacks=[train_log, lr_schedule,  early_stopping])
    ...
    def cluster(self, args, text,x_data, y_data=None, test="train", tol=0.01, iter_max=1e6, **kwargs):

        save_path = os.path.join(args.save_weight_path, "dec_weights_{}.h5")
        ...
        if test=="test":
            ...
            # Create CSV file for the current cluster
                csv_file_path = os.path.join('C:/Users/madhu/Desktop/SeattleUniversity/capstone/resonate_madhu/Resonate/Notebooks/AE_clustering', f"cluster_{cluster_id}.csv")
                with open(csv_file_path, mode='w', newline='') as file:
                    writer = csv.writer(file)
                    writer.writerow(["Text","Cluster Label"])

                # Write text data and corresponding cluster labels to the CSV file
                    for i in range(len(x_data)):
                        if self.y_pred[i] == cluster_id:
                            writer.writerow([text[i], self.y_pred[i]])
